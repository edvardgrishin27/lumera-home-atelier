...
const Home = () => {
    ...
    // Quote HTML with accent styling - content is from trusted CMS defaults only
    // Safe: content comes from defaultContent in ContentContext, not from user input
    ...
    return (
        <div ref={containerRef} className="w-full bg-background overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-16 px-6 md:px-20">
                    {hitProducts.map((product, idx) => (
                        <Link
                            ...
                            className="hit-card opacity-0 translate-y-10 group cursor-pointer block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent rounded-2xl"
                        >
            ...
            <section className="py-40 px-6 md:px-20 text-center bg-background relative z-10 content-layer">
                {/* Safe: quoteHtml is built from trusted CMS default content, not user input */}
                <p
